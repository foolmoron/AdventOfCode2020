...
function parse(rules) {
    return rules
        ...
        .map(([main, inner]) => ({
            main,
            children: inner === 'no other bags.' ? [] : inner
                .split(', ')
                .map(bagConfig => ({
                    count: parseInt(bagConfig[0]),
                    bag: / (.*) bag/.exec(bagConfig)[1],
                }))
        }))
        .reduce((acc, i) => ({ ...acc, [i.main]: i.children }), {})
}

function canContain(map, bag, goal) {
    // console.log(bag, map[bag].map(i => i.bag))
    if (map[bag].find(inner => inner.bag === goal)) {
        return true
    }
    return map[bag].some(inner => canContain(map, inner.bag, goal))
}

const m = parse(input)
console.log(Object.keys(m).filter(bag => canContain(m, bag, 'shiny gold')).length)

function bagSum(map, bag) {
    const thisSum = map[bag].reduce((acc, i) => acc + i.count, 0)
    const innerSum = map[bag]
        .map(inner => inner.count * bagSum(map, inner.bag))
        .reduce((acc, i) => acc + i, 0)
    return thisSum + innerSum
}

const m2 = parse(input)
console.log(bagSum(m2, 'shiny gold'))
