...
function sumAll(exps) {
    return exps.reduce((acc, i) => acc + calc(i), 0)
}

console.log(sumAll(parse(input)))
...
function calc2(exp) {
    while (exp.includes('(')) {
        ...
        for (let i = 0; i < exp.length; i++) {
            ...
            if (brackets === 0 && bracketStart !== null) {
                const val = calc2(exp.substring(bracketStart + 1, i))
                exp = exp.substring(0, bracketStart) + val + exp.substring(i + 1)
                break
            }
        }
    }
    while (exp.includes('+')) {
        const m = /(\d+)\+(\d+)/.exec(exp)
        const val = parseInt(m[1]) + parseInt(m[2])
        exp = exp.substring(0, m.index) + val + exp.substring(m.index + m[0].length)
    }
    while (exp.includes('*')) {
        const m = /(\d+)\*(\d+)/.exec(exp)
        const val = parseInt(m[1]) * parseInt(m[2])
...
function sumAll2(exps) {
    return exps.reduce((acc, i) => acc + calc2(i), 0)
}
...
console.log(sumAll2(parse(input)))
