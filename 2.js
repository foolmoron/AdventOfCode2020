...
function countValid(passwords) {
    const valid = passwords.filter(p => {
        const count = p[3].split(p[2]).length - 1
        return p[0] <= count && count <= p[1]
    })
    return valid.length
}

function countValid2(passwords) {
    const valid = passwords.filter(p => {
        const a = p[3][p[0] - 1] == p[2]
        const b = p[3][p[1] - 1] == p[2]
        return a ^ b
    })
    return valid.length
}
