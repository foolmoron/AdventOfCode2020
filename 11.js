...
function parse(seats) {
    return seats.trim().split('\n')
        .map(line => line.split('')
            .map(seat => seat === 'L' ? 0 : -1)
        )
}

function countAdjacentOccupied(seats, x, y) {
    let count = 0
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            if (i !== 0 || j !== 0) {
                count += Math.max(0, (seats[y + j] || [])[x + i] || 0)
            }
        }
    }
    return count
...
function simulate(seats) {
    ...
    for (let y = 0; y < newSeats.length; y++) {
        for (let x = 0; x < newSeats[y].length; x++) {
            const seat = newSeats[y][x];
            if (seat === 0) {
                if (countAdjacentOccupied(seats, x, y) === 0) {
                    newSeats[y][x] = 1
                    changed = true
                }
            } else if (seat === 1) {
                if (countAdjacentOccupied(seats, x, y) >= 4) {
                    newSeats[y][x] = 0
                    changed = true
                }
            }
...
function countVisibleOccupied(seats, x, y) {
    let count = 0
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
                if (i !== 0 || j !== 0) {
                ...
                while (true) {
                    tryX += i
                    tryY += j
                    const seat = (seats[tryY] || [])[tryX]
                    if (seat >= 0 || seat === undefined) {
                        count += Math.max(0, seat || 0)
                        break
                    }
                }
            }
...
function simulate2(seats) {
    ...
    for (let y = 0; y < newSeats.length; y++) {
        for (let x = 0; x < newSeats[y].length; x++) {
            const seat = newSeats[y][x];
            if (seat === 0) {
                if (countVisibleOccupied(seats, x, y) === 0) {
                    newSeats[y][x] = 1
                    changed = true
                }
            } else if (seat === 1) {
                if (countVisibleOccupied(seats, x, y) >= 5) {
                    newSeats[y][x] = 0
                    changed = true
                }
            }
