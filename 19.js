...
function parse(z) {
    const gram = z.split('\n\n')[0].trim().split('\n').map(line => {
            ...
            const rule = rest.includes('"')
                ? eval(rest)
                : rest.split(' | ').map(x => x.split(' ').map(i => parseInt(i)))
            return {
                index: parseInt(num),
                rule: rule
            }
        }).reduce((acc, i) => ({...acc, [i.index]: i.rule}), {})
    const strs = z.split('\n\n')[1]
    return {
        gram: gram,
        strs: strs.trim().split('\n')
    }
}

function getRegex(gram, index, memo) {
    if (!memo[index]) {
        const rules = gram[index]
        if (typeof rules === 'string') {
            memo[index] = rules
        } else {
            memo[index] = '(?:' + rules.map(rule => rule.map(num => getRegex(gram, num, memo)).join('')).join('|') + ')'
        }
    }
    return memo[index]
}

function matches(gram, str) {
    const memo = {}
    return new RegExp('^' + getRegex(gram, 0, memo) + '$').exec(str)
}

function countMatches(z) {
    return z.strs.filter(s => matches(z.gram, s)).length
}

console.log(countMatches(parse(input)))
...
function matches2(gram, str, memo) {
    const match = new RegExp('^' + getRegex(gram, 0, memo) + '$').exec(str)
    return match && (match[1] || {}).length === (match[2] || {}).length
}

function countMatches2(z) {
    const memo = {}
    memo[8] = '(?:' + getRegex(z.gram, 42, memo) + ')+'
    memo[11] = '(?:' + Array.from(new Array(30)).map((_,i) => getRegex(z.gram, 42, memo).repeat(i + 1) + getRegex(z.gram, 31, memo).repeat(i + 1)).join('|') + ')'
    return z.strs.filter(s => matches2(z.gram, s, memo)).length
}

console.log(countMatches2(parse(input)))
